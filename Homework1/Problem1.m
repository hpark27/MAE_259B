...
% Both implicit and explicit method are used for the simulation
...
% viscous damping
% update viscous damping on each sphere
C = zeros(2*N,2*N);

% viscous damping on each sphere
c1 = 6*pi*mu*r1;
c2 = 6*pi*mu*r2;
c3 = 6*pi*mu*r3;
...
% error tolerance value for the loop for Newton Raphson method
e = EI/l^2*1e-3;
...
x2(1) = q(2*midN); % update initial position
